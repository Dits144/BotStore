const { getActiveQuiz, clearActiveQuiz } = require('../services/quizService');
...
module.exports = {
  ...
  async execute({ sock, jid, sender, args }) {
    ...
    const correct = userAnswer === activeQuiz.answer.toLowerCase();
    if (correct) {
      const user = addXpAndScore(sender, 15);
      updateStreak(sender);
      clearActiveQuiz(sender, { question: activeQuiz.question, result: 'correct', answer: userAnswer });
    ...
    clearActiveQuiz(sender, { question: activeQuiz.question, result: 'wrong', answer: userAnswer });
    await sock.sendMessage(jid, {
